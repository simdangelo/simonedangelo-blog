...
**Critically, the query doesn't access the heap (the main table storage).** Since only `id` is requested and `id` exists in the index, the database retrieves the value directly from the index—an "inline query." This is the fastest possible scenario: finding data without touching the main table.
...
The database now performs a **bitmap index scan** on `employees_name_idx`, locating matching rows via the index rather than scanning the entire table. While not as fast as querying the primary key (which benefits from being clustered), 47ms is dramatically better than 3 seconds.
...
# How Database Optimizers Decide to Use Indexes (NEW)
