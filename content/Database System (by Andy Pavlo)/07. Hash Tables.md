...
# Data Structures
We need **data structures** for almost every part of the system:
- **Internal metadata**, such as a **page directory**, which maps a **page ID** to its location.
- **Index-organized tables**, where the data structure itself represents the **stored data**.
- **Temporary data structures**, often created for quick operations and discarded afterward. Many queries build a **hash table** on the fly, process it, and then discard it.
- **Table indexes**, used for mapping keys to records, including primary keys and secondary keys. In MySQL, for example, secondary indexes point to the primary key, which is then used for a second lookup.
