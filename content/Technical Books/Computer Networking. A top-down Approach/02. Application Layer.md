...
This section introduces the **Domain Name System (DNS)**, the crucial directory service that makes the Internet usable for humans by bridging the gap between memorable names and necessary addresses.

The Internet identifies hosts in two main ways:
1. **Hostnames:** Mnemonic names (like `www.google.com` or `www.eurecom.fr`) that are easy for humans to remember. However, they provide little information about the host's physical location (`.fr` tells us that the host is probably in France) and are variable-length, making them hard for routers to process.
2. **IP Addresses:** Fixed-length, **hierarchically structured** numerical identifiers (like `121.7.106.83`). These 4-byte addresses, each separated by a period, can be from 0 to 255, and they're what routers prefer because they provide specific location information necessary for routing packets. 
## 2.4.1 Services Provided by DNS
The **Domain Name System (DNS)** exists to reconcile Hostnames and IP Addresses. DNS performs its main task—**translating hostnames to IP addresses**—by being two things simultaneously:
1. A **distributed database** implemented in a hierarchy of DNS servers.
2. An **application-layer protocol** that allows hosts to query this database.

The DNS servers are often UNIX machines, and the DNS protocol runs over **UDP** and uses **port 53**.

DNS is heavily used by other application-layer protocols (like HTTP and SMTP). Here is how a translation occurs when a browser requests a URL like `www.someschool.edu/index.html`:
1. The same user machine runs the client side of the DNS application.
	+ It's a **piece of software or service built into your operating system** (like Windows, macOS, or Linux) that manages the process of finding an IP address. It's often referred to as the DNS Resolver or Stub Resolver.
...
This lookup adds an extra delay, but frequently used addresses are **cached** in nearby DNS servers to reduce both delay and network traffic.

DNS provides several essential services beyond simple translation:
...
- **Load Distribution:** For highly visited sites (like `cnn.com`), multiple replicated servers exist, each with a different IP address. DNS stores this **set of IP addresses** for the single alias name. When queried, the DNS server responds with the full set of addresses but **rotates their order** in each reply. Since clients usually connect to the address listed first, this rotation mechanism effectively **distributes the traffic load** among the replicated servers.
...
From the perspective of an application (like a browser), DNS is a simple service: you give it a hostname, and it returns an IP address. This translation is handled by a function call (e.g., `gethostbyname()` on many UNIX-based machines), which sends a query message over the network using UDP datagrams to port 53.

While a simple, single, centralized DNS server would be easy to design, it is entirely impractical for the scale of the modern Internet. A centralized system fails due to:
...
Therefore, the DNS is a **distributed, hierarchical database** spread across thousands of servers worldwide.
...
1. **Root DNS Servers:** These sit at the very top of the hierarchy. There are over 1,000 physical server instances globally, derived from **13 logical root servers** coordinated by organizations like the Internet Assigned Numbers Authority (IANA). The Root Servers do not know the IP address for any specific host; their sole job is to provide the IP addresses of the **Top-Level Domain (TLD) servers** for the domain suffix being queried (e.g., they point to the servers for `.com`, `.org`, or `.edu`).
2. **Top-Level Domain (TLD) Servers:** For every top-level domain (`.com`, `.net`, `.gov`, and country-code domains like `.fr`, `.jp`), there is a TLD server or a cluster of servers (like the servers managed by Verisign for `.com`). The TLD servers maintain the records necessary to point a query toward the next level: the **Authoritative DNS servers**.
3. **Authoritative DNS Servers:** These are the **final authority** for a specific organization's domain (e.g., `amazon.com`). Every company or institution with publicly accessible hosts (like web or mail servers) must register and maintain these records in their Authoritative DNS server. This server is the only one that knows the specific, detailed mappings that translate an organization's hostname (e.g., `www.amazon.com`) into its corresponding IP address.
...
In this full example, **eight DNS messages** (four queries, four replies) were exchanged just to get a single IP address!

This example makes use of both recursive queries and iterative queries:
...
* The subsequent three queries are **iterative** since all of the replies are directly returned to `dns.nyu.edu`.
### DNS Caching
